...
import { AlertTriangle, CheckCircle2, Circle, Clock, Plus, Flag, Building, GraduationCap, Plane, FileText, Trash2 } from 'lucide-react'
import { getLocalData, addItem, updateItem, deleteItem, saveLocalData } from '../lib/storage'
...
export default function Deadlines() {
  ...
  const loadDeadlines = async () => {
    ...
    if (stored.length > 0) {
      ...
      // Save initial deadlines to localStorage on first load
      await saveLocalData('deadlines', initialDeadlines)
        ...
        <div className="bg-white border border-slate-200 rounded-2xl p-5 shadow-sm animate-slide-up">
          ...
          <div className="grid grid-cols-1 md:grid-cols-5 gap-3">
            <input type="text" value={newDeadline.title} onChange={(e) => setNewDeadline({ ...newDeadline, title: e.target.value })} placeholder="Title"
              className="md:col-span-2 px-3 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-sm outline-none focus:ring-2 ring-blue-500/20 focus:border-blue-300" />
            ...
            <button onClick={handleAddDeadline} disabled={isLoading} className="px-4 py-2.5 bg-slate-900 text-white text-sm font-medium rounded-xl hover:bg-slate-800 disabled:opacity-50 transition-colors">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {overdue.length > 0 && (
            <div className="bg-red-50 border border-red-200 rounded-2xl p-5">
              ...
              <div className="space-y-2">
                {overdue.map((d) => (
                  <div key={d.id} className="flex items-center justify-between p-2.5 bg-white/80 rounded-lg border border-red-100">
          ...
          {upcoming.length > 0 && (
            <div className="bg-amber-50 border border-amber-200 rounded-2xl p-5">
              ...
              <div className="space-y-2">
                {upcoming.map((d) => (
                  <div key={d.id} className="flex items-center justify-between p-2.5 bg-white/80 rounded-lg border border-amber-100">
        ...
        {['All', 'Tax', 'Visa', 'University', 'Corporate'].map((cat) => (
          <button key={cat} onClick={() => setFilter(cat)}
            className={`px-3.5 py-1.5 text-xs font-medium rounded-lg transition-all ${
              filter === cat ? 'bg-slate-900 text-white shadow-sm' : 'bg-white text-slate-500 border border-slate-200 hover:border-slate-300'
            }`}>{cat}</button>
        ))}
      </div>
        ...
        <div className="space-y-0">
          {sorted.length === 0 ? (
            <div className="text-center py-12 text-slate-400">
              ...
              <p className="text-xs mt-1">Add a deadline to get started</p>
            </div>
            ...
            return (
              <div key={deadline.id || i} className={`flex items-center gap-4 p-4 border-b border-slate-100 last:border-0 hover:bg-slate-50 transition-all group ${deadline.done ? 'opacity-50' : ''}`}>
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2 mb-0.5">
                    <p className={`text-sm font-semibold ${deadline.done ? 'line-through text-slate-400' : 'text-slate-900'}`}>
                      {deadline.title}
                    </p>
                    <Flag size={12} className={priorityColors[deadline.priority]} />
                  </div>
                ...
                <div className="text-right shrink-0">
                  ...
                  <p className={`text-xs font-semibold mt-1 ${
                    isOverdue ? 'text-red-500' : days <= 30 ? 'text-amber-500' : 'text-slate-400'
                  }`}>
                    {deadline.done ? 'Completed' : isOverdue ? `${Math.abs(days)}d overdue` : `${days}d left`}
                  </p>
                </div>
                <button onClick={() => handleDelete(deadline.id)} className="opacity-0 group-hover:opacity-100 w-9 h-9 flex items-center justify-center rounded-lg text-red-500 hover:bg-red-50 transition-all shrink-0">
                  <Trash2 size={16} />
                </button>

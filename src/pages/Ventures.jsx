...
import {
  ...
  ArrowDownRight, Filter, ChevronDown, Plus, X,
} from 'lucide-react'
...
export default function Ventures() {
  ...
  useEffect(() => {
    // Load ventures from localStorage
    loadVentures()
  }, [])
        ...
        <div className="bg-white border border-slate-200/80 rounded-2xl p-5 shadow-sm animate-slide-up">
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 gap-3">
            <input
              ...
              className="px-3 py-2 bg-slate-50 border border-slate-200 rounded-lg text-sm outline-none focus:ring-2 ring-blue-500/20"
            />
            <select
        ...
        {filtered.map((venture) => {
          ...
          return (
            <div
              ...
              className={`bg-white border-2 rounded-2xl p-6 shadow-sm hover:shadow-md transition-all cursor-pointer ${
                selectedVenture === venture.id ? 'border-blue-500' : 'border-slate-200'
              }`}
            >
              <div className="flex items-start justify-between mb-4">
                <div
                  className="w-10 h-10 rounded-lg flex items-center justify-center"
                  style={{ backgroundColor: venture.bgColor || '#f1f5f9' }}
                >
                ...
                <div className="flex gap-px items-end h-8">
                  {[40, 60, 45, 70, 55, 80].map((h, i) => (
                    <div
                      ...
                      style={{
                        height: `${h * 0.3}px`,
                        backgroundColor: i === 5 ? venture.color : '#f1f5f9',
                        opacity: i === 5 ? 0.8 : 1,
                      }}
                    ></div>
                  ))}
      ...
      {detail && (
        <div className="bg-white border border-slate-200/80 rounded-2xl p-6 shadow-sm">
          <div className="flex items-center justify-between mb-6">
            ...
            <button onClick={() => setSelectedVenture(null)} className="text-slate-400 hover:text-slate-600">
              <X size={20} />
            </button>
          </div>

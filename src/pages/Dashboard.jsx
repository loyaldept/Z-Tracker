    ...
    <div className="space-y-8 animate-fade-in">
      <div className="bg-gradient-to-r from-slate-900 via-slate-800 to-slate-900 rounded-2xl p-6 relative overflow-hidden">
        <div className="relative z-10 flex items-center gap-4">
          ...
          <p className={`text-white/90 text-sm lg:text-base font-medium italic transition-all duration-500 ${quoteVisible ? 'opacity-100' : 'opacity-0'}`}>
            "{displayQuote}"
          </p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        {kpiCards.map((card) => {
          ...
          return (
            <div key={card.label} className="bg-white border border-slate-200/80 rounded-2xl p-5 shadow-sm hover:shadow-md transition-all">
              <div className="flex items-center justify-between mb-3">
                <div className={`w-9 h-9 rounded-xl ${colors.bg} flex items-center justify-center`}>
                  <Icon size={16} className={colors.text} />
                </div>
                <span className={`text-[10px] font-bold ${colors.text} ${colors.bg} px-2 py-0.5 rounded-full`}>{card.change}</span>
              </div>
      ...
      <div className="grid grid-cols-12 gap-6">
        ...
        <div className="col-span-12 lg:col-span-4 space-y-6">
          <div className="bg-slate-900 rounded-2xl p-6 text-white relative overflow-hidden">
            <div className="relative z-10">
              ...
              <div className="pt-4 mt-4 border-t border-white/10 flex justify-between items-center">
                <div>
                  <span className="text-[10px] font-bold text-slate-500 uppercase tracking-widest block">Monthly Burn</span>
                  <span className="text-sm font-semibold">$0/mo</span>
                </div>
                <Link to="/ventures" className="w-8 h-8 rounded-full bg-white/10 flex items-center justify-center hover:bg-white/20 transition-all">
                  <ArrowRight size={14} />
                </Link>
              </div>
            </div>
          ...
          <Link to="/visa" className="block bg-white border border-slate-200/80 rounded-2xl p-6 shadow-sm hover:shadow-md transition-all group">
            <div className="flex items-center justify-between mb-3">
            ...
            <div className="h-1.5 w-full bg-slate-100 rounded-full mt-4">
              <div className="h-full bg-gradient-to-r from-blue-500 to-indigo-500 w-[35%] rounded-full"></div>
            </div>
        ...
        <div className="col-span-12 md:col-span-7 lg:col-span-8">
          ...
          <div className="grid grid-cols-2 lg:grid-cols-3 gap-3">
            {ventures.map((v) => {
              ...
              return (
                <Link key={v.id} to="/ventures" className="bg-white border border-slate-200/80 rounded-xl p-4 shadow-sm hover:shadow-md hover:border-slate-300 transition-all">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <Link to="/jamka" className="bg-white border border-slate-200/80 rounded-2xl p-6 shadow-sm hover:border-rose-200 transition-all group relative overflow-hidden">
        ...
        <Link to="/university" className="bg-white border border-slate-200/80 rounded-2xl p-6 shadow-sm hover:border-indigo-200 transition-all group">
          <div className="flex items-center gap-3 mb-4">
        ...
        <Link to="/deadlines" className="bg-white border border-slate-200/80 rounded-2xl p-6 shadow-sm hover:border-amber-200 transition-all group">
          <div className="flex items-center gap-3 mb-4">

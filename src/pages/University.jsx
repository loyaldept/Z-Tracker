  ...
  return (
    <div className="space-y-8 animate-fade-in">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        {degrees.map((deg) => (
          <div key={deg.id} className="bg-white border border-slate-200 rounded-2xl p-6 shadow-sm">
            <div className="flex items-center justify-between mb-4">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <p className="text-sm font-bold text-slate-900">{deg.school}</p>
                  <p className="text-xs text-slate-500">{deg.location}</p>
                </div>
              </div>
              <span
                className="text-[10px] font-bold px-2 py-0.5 rounded-full border"
                style={{ backgroundColor: `${deg.color}10`, color: deg.color, borderColor: `${deg.color}30` }}
              >
                {deg.status}
              </span>
            </div>
            ...
            <div className="space-y-2 mb-4">
              {[
                ...
                <div key={label} className="flex justify-between text-xs p-2 bg-slate-50 rounded-lg border border-slate-100">
            ...
            {deg.seniorProject && (
              <div className="mt-4 p-3 bg-indigo-50 rounded-xl border border-indigo-100">
                ...
                <p className="text-xs text-slate-600">Currently working on Senior Project — Senior I semester</p>
              </div>
            )}
      ...
      <div className="bg-white border border-slate-200 rounded-2xl p-6 shadow-sm">
        <h3 className="text-base font-bold text-slate-900 mb-1">Credit Accumulation</h3>
        ...
        <ResponsiveContainer width="100%" height={250}>
          <BarChart data={creditData}>
            ...
            <Tooltip
              contentStyle={{ background: 'white', border: '1px solid #e2e8f0', borderRadius: '12px', fontSize: '11px', color: '#0f172a' }}
            />
      ...
      {/* Timeline */}
      <div className="bg-white border border-slate-200 rounded-2xl p-6 shadow-sm">
        <h3 className="text-base font-bold text-slate-900 mb-6">Academic Timeline</h3>
            ...
            {timeline.map((item, i) => (
              <div key={i} className="flex items-start gap-4 relative">
                <div className={`w-8 h-8 rounded-full flex items-center justify-center shrink-0 z-10 ${
                  item.done ? 'bg-green-50' : item.icon === 'current' ? 'bg-blue-50' : 'bg-slate-100'
                }`}>
                  {item.done ? (
                ...
                <div className={`flex-1 pb-4 ${!item.done && item.icon !== 'current' ? 'opacity-40' : ''}`}>
                  <p className="text-xs text-slate-500 font-medium mb-0.5">{item.date}</p>
